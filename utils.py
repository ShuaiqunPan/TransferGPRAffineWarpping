...
def evaluation(store_MAPE, store_SMAPE, store_R_square, store_log_ratio_score, store_square_sum_log_ratio_score):
    ...
    print('-------------------------------------------------------------------------------------------------')
    print('-------------------------------------------------------------------------------------------------')
    return SMAPE_final, SMAPE_final_std
...
def write_csv_header(csv_path):
    ...
    header = ["BBOB Function"] + [f"Run {i+1}" for i in range(10)] + ["Average SMAPE"]
    with open(csv_path, "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(header)

def append_csv_row(csv_path, function_name, smape_values):
    """Append a row of SMAPE values for a BBOB function to the CSV file."""
    average_smape = sum(smape_values) / len(smape_values)
    row = [function_name] + smape_values + [average_smape]
    with open(csv_path, "a", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(row)

def get_hyperparameters_from_csv(csv_path, function, setting, run):
    """
    Fetch hyperparameters from the CSV file for a given function, setting, and run.
    """
    with open(csv_path, 'r') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            if (
                row['Function'] == function and
                row['Setting'] == str(setting) and
                row['Run'] == f'run{run}'
            ):
                return {
                    'lr': float(row['lr']),
    ...
    raise ValueError(f"No matching hyperparameters found for {function}, {setting}, run{run}")